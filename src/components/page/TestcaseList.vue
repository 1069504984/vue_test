...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleRun(index, row) {
                this.idx = index;
                console.log('当前运行的下标', this.idx); // 当前修改的数据, 在tableData数组中的索引值
                this.id = row.id;
                console.log('当前运行的id', this.id);// 当前修改的数据在数据库中的真实索引值
                this.form = row;
                console.log('当前运行的form', this.form);
            ...
            getEnvsIdNames() {
                envs_names()
                    .then(response => {
                        console.log("create时期调用getEnvsIdNames",response)
            ...
            //
            confirmRun() {
                console.log('此时的环境id',this.env_id)
                run_by_testcase(this.id, this.env_id)
                    .then(response => {
                        console.log('运行接口的返回值',response.data);   // 通过项目运行用例
