        ...
        data: function(){
            return {
                ...
                form: {
                    ...
                    project_id: '',    // 项目ID
                    include: '[]',   // 接口ID列表字符串
                    selected_intercase_id_name_list: []  //接口id和nmae
                },
                rules: {
                    name: [
        ...
        methods: {
            onSubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        let that = this;
                        console.log("编辑套件提交更新的参数",this.form),
                        console.log("编辑套件提交更新的套件id",this.current_testsuite_id),
            ...
            getTestSuiteDetail(){
                get_detail_testsuite(this.current_testsuite_id)
                    .then(response => {
                        ...
                        this.form.selected_intercase_id_name_list = response.data.selected_intercase_id_name_list
                        this.selected = response.data.selected_intercase_id_name_list
